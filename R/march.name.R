#' Match original data and geometry data
#'
#' @param data original data
#' @param place place names of original data
#' @param geometry geometry data, default is map data with geometry column names of Name
#' @param choice a numeric vector, default is NULL
#' @return a dataframe of the original data and geometry data.
#' @importFrom do %==%
#' @importFrom crayon red bold
#' @export
match.name <- function(data,place,geometry=mapdata,choice=NULL){
    name=as.character(data[,place])
    check= name %==% geometry$Name
    CHECK=FALSE
    if (length(check)==0){
        model.check=1
        CHECK=TRUE
    ...
    if (CHECK) {
        ...
        for (i in 1:length(unpaired)) {
            ...
            for (j in 1:length(unp.i)) {
                ...
                res.tump=c(res.tump,unique(geometry$Name[grepl(unp.i[j],geometry$Name)]))
            }
    ...
    check=as.character(name) %==% geometry$Name
    CHECK=FALSE
    if (length(check)==0){
        model.check=1
        CHECK=TRUE
    ...
    loc=unlist(name %==% geometry$Name)
    map.ready0=unique(geometry[loc,])
    #check dup in quxian
    ...
    map.ready1=merge(data,map.ready0,all = TRUE)
    cat('\n')
    st_as_sf(map.ready1[,set::not(names(map.ready1),'Code')],sf_column_name = 'geometry')
}
